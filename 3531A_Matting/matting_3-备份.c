...
void *mattingProcessThread(void *argv)
{
	...
	readBmp(pRgb, "/mnt/disk/org.bmp");
	HI_MPI_SYS_MflushCache(u32PhyAddr_RGB, pRgb, 1920*1080*3);

	img2Yuv(u32PhyAddr_RGB, &stFrame_img);
	img2Hsv(&stFrame_img, u32PhyAddr_HSV);
	img2Lab(&stFrame_img, u32PhyAddr_LAB);

	HI_MPI_VPSS_SendFrame(12, &stFrame_img, 1000);
